...
# <<< THIS IS THE MAGIC NUMBER - DO NOT CHANGE >>>
LATENT_SCALE_FACTOR = 2.874741   # ← 99% of flat loss comes from forgetting this
...
def main():
    ...
    parser.add_argument('--in_channels', type=int, default=8)  # Custom VAE has 8 channels
    parser.add_argument('--T', type=int, default=16)
    parser.add_argument('--H', type=int, default=64)          # 512 / 8 = 64
    parser.add_argument('--W', type=int, default=64)          # 512 / 8 = 64
    ...
    sample = dataset[0]
    print(f"Raw latent shape: {sample.shape}")          # Should be [8, 16, 64, 64] or [8, 17, 64, 64] is bad
    ...
    # <<< CORRECT TINY VAE CONFIG - MUST MATCH generate_latents.py >>>
    vae_config = {
    ...
    for epoch in range(args.epochs):
        ...
        for latents in progress_bar:
            latents = latents.to(device)                               # [B,8,16,64,64]

            # <<< THIS LINE IS ABSOLUTELY MANDATORY >>>
            latents = latents / LATENT_SCALE_FACTOR

            # Optional but extremely useful debug (keep for first 5 epochs)
            if epoch < 5 and progress_bar.n == 0:
                print(f"Latent std after scaling: {latents.std().item():.4f} (should be ~1.0)")
