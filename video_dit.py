...
# rotary embedding nos permite proveer a ala atencion la posicion de los tokens, sin sumar un positional encoding
# se lo hace con un embedding de frecuencias, que se aplica a la query y key, para luego rotar la key y la query
...
#version generica y reusable de atencion
#la atencion temporal y espacial cambia solo dependiendo de como se la llama
#softmax(q*t^k/sqrt(dk))v
...
#ml de dos capas con gelu y dropout
...
#bloque de diffusion transfoemre RMS->Attention->RMS->FeedForward->adaLN
#diferencias con transformer normal: usa RMS norm en lgar de layernorm, adaLN, gating
class DiTBlock(nn.Module):
 ...
 #shift es usado en adaln, adaln es un "knob" que decide en cada timestep, que canales deben ser amplificados, suprimidos o no afectados.
 #adaln es un cheatcode que le permite al transformer"arreglar" solo lo que se necesita en ese timestep.
 #gates son usadas para selectivamente aplicar o no atencion.
...
#bloque espacio temporal, la diferencia entre el bloque de espacio y de tiempo, es que en espacio
# se calcula atencion en el mismo frame a lo largo de patches vecinos, en temporal e calcula atencion a lo largo de diferentes frames de la misma ubicacion
